...
MeshFactory::size_type MeshFactory::AddPoint(coord_t coord) {
  ...
  polytopicmesh::Mesh::GeometryPtr point_geo =
...
std::shared_ptr<mesh::Mesh> MeshFactory::Build() {
  ...
  mesh::Mesh* mesh_ptr =
      new polytopicmesh::Mesh(dim_world_, std::move(nodes_), std::move(edges_),
                         std::move(elements_), check_completeness_);

  // Clear all information supplied to the MeshFactory object
  nodes_ = polytopicmesh::Mesh::NodeCoordList{};  // .clear();
  edges_ = polytopicmesh::Mesh::EdgeList{};       // .clear();
  elements_ = polytopicmesh::Mesh::CellList{};    // clear();
...
void MeshFactory::PrintLists(std::ostream& o) const {
  o << "polytopicmesh::MeshFactory: Internal information" << std::endl;
